...
pegawai.socket = function(io, connections, client) {
    ...
    client.on('edit_pegawai', function(data, cb) {
        if (data.type == 'pegawai') {
            ...
            CustomEntity.update({ _id: data._id }, {
                [data.field]: data.value
            }, function(err, status) {
                if (err) {
                    cb('gagal');
...
pegawai.post('/ajax/edit', function(req, res) {
    ...
    MongoClient(url, { useNewUrlParser: true, useUnifiedTopology: true }).connect(function(err, db) {
        if (err) {} else {
            ...
            db.collection('pegawai').update({ "nip": req.body.nip }, {
                $set: {
                    [jenis]: value
                }
            }, function(err, result) {
