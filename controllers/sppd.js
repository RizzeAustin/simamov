            ...
            async.series([
                ...
                XlsxPopulate.fromFileAsync("./template/spj_template/rekap_sppd.xlsx")
                    .then(workbook => {
                        ...
                        //=>bendahara
                        row += 2;
                        workbook.sheet(0).range('A' + row + ':A' + (row + 6)).value([
                            ['Lunas pada tanggal'],
                            ['Bendahara Pengeluaran STIS,'],
                            [''],
                            [''],
                            [''],
                            [setting.bendahara.nama],
                            ['NIP ' + setting.bendahara._id]
                        ]);
                        //=>>style
                        workbook.sheet(0).cell('A' + (row + 5)).style('underline', true);

                        //=>ppk
                        workbook.sheet(0).range('E' + (row + 1) + ':E' + (row + 6)).value([
                        ...
                        //=>pembuat daftar
                        workbook.sheet(0).range('I' + row + ':I' + (row + 6)).value([
                            ['Jakarta, ' + data.tgl_buat_rekap],
                            ['Pembuat Daftar,'],
                            [''],
                            [''],
                            [''],
                            [data.pembuat_daftar],
                            ['NIP ' + data.pembuat_daftar_id]
                        ]);
                        //=>>style
    ...
    async.series([
                        ...
                        st.save(function(err, result) {
                            st.populate('nama_lengkap ttd_legalitas ttd_surat_tugas prov kab org', function(err2) {
                                ...
                                async.series([
                                    ...
                                    function assignData() {
                                        data.yang_bepergian.push({
                                            nomor_surat: st.nomor_surat,
                                            nama: st.nama_lengkap.nama,
                                            _id: st.nama_lengkap._id,
                                            gol: st.nama_lengkap.gol,
                                            jabatan: st.nama_lengkap.jabatan
                                        })
        ...
        function(err, finish) {
            User.update({ _id: user_id }, { $push: { "act": { label: 'Buat Surat Tugas ' + data.nomor } } },
                function(err, status) {})
            CustomEntity.update({ 'nama': data.tugas, type: 'tugas' }, { $setOnInsert: { 'nama': data.tugas } }, { upsert: true },
...
function handleSuratTugasBiasa(data, cb, res, user_id) {
    ...
    async.series([
            ...
            function(cb) {
                CustomEntity.update({ 'nama': data.lokasi, type: 'lokasi' }, { $setOnInsert: { 'nama': data.lokasi } }, { upsert: true },
                    function(err, numAffected) {
                        cb(null, '')
                    }
                );
                                    ...
                                    _.each(result.realisasi, function(real, idx, list) {
                                        if (real.ket.match(new RegExp('SPPD No\. ' + st._id + '\:', "i"))) {
                                            DetailBelanja.findOneAndUpdate({ 'thang': thang, "_id": data.detail, 'realisasi._id': real._id }, {
                                                $set: {
                                                    'realisasi.$.pengentry': new_entry.pengentry,
                                                    'realisasi.$.ket': new_entry.ket,
                                                    'realisasi.$.penerima_nama': new_entry.penerima_nama,
                                                    'realisasi.$.tgl': new_entry.tgl,
                                                    'realisasi.$.tgl_timestamp': new_entry.tgl_timestamp,
                                                    'realisasi.$.penerima_id': new_entry.penerima_id,
                                                    'realisasi.$.jumlah': new_entry.jumlah,
                                                    'realisasi.$.timestamp': new_entry.timestamp
                                                }
                                            }, function(err, result) {
                                                console.log(result)
